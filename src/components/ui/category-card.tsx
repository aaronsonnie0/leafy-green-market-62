...
interface CategoryCardProps {
  ...
  link: string;
  bgColor: string;
  iconName: "plant" | "pot" | "accessory" | "featured";
  delay?: number;
}
...
export function CategoryCard({ 
  ...
  const getIcon = () => {
    switch (iconName) {
      ...
      case "accessory":
        return <Leaf className="h-8 w-8 text-leaf-600" />;
      case "featured":
        return <Flower2 className="h-8 w-8 text-leaf-600" />;
      default:
        return <Leaf className="h-8 w-8 text-leaf-600" />;
    }
  };
  ...
  return (
    ...
    >
      <div className="w-16 h-16 flex items-center justify-center rounded-full bg-white/50 dark:bg-white/10 mb-5">
        {getIcon()}
      </div>
